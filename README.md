...
- Supabase 공식 Next.js 가이드 패턴 적용:
  - `@supabase/ssr` 패키지 사용 (cookie 기반 세션 관리)
  - Next.js 15의 `cookies()` 사용
  - Server Component에서 최적화된 데이터 쿼리
- 환경별 Supabase 클라이언트 분리:
  - Client Component용 (`useClerkSupabaseClient`)
  - Server Component용 (`createClerkSupabaseClient`) - 공식 가이드 패턴
...
2. 프로젝트 선택 후 **"Setup"** 또는 **"Integrations"** 메뉴로 이동
3. **"Supabase"** 통합 찾기 또는 [Supabase integration setup 페이지](https://dashboard.clerk.com/setup/supabase)로 직접 이동
4. 통합 옵션 선택 후 **"Activate Supabase integration"** 클릭
5. **Clerk domain**이 표시되면 복사해두세요 (예: `your-app-12.clerk.accounts.dev`)
   - 이 도메인은 다음 단계에서 사용됩니다
   - 이 단계에서 Clerk 세션 토큰에 `role: "authenticated"` 클레임이 자동으로 추가됩니다
...
- 통합이 완료되면 Clerk 세션 토큰이 Supabase에서 자동으로 검증됩니다
- JWT 템플릿이나 추가 설정 없이 바로 사용할 수 있습니다
...
**테스트 페이지:**
- `/instruments`: Supabase 데이터 쿼리 예시 (공식 가이드 기반)
...
프로젝트에 이미 Clerk 한국어 로컬라이제이션이 적용되어 있습니다. [Clerk 공식 로컬라이제이션 가이드](https://clerk.com/docs/guides/customizing-clerk/localization)의 모범 사례를 따릅니다.

**현재 설정:**

- `@clerk/localizations` 패키지 사용
- `koKR` 로컬라이제이션 적용 (`app/layout.tsx`)
...
const customKoKR = {
  ...
  unstable__errors: {
    ...
    not_allowed_access: "접근이 허용되지 않은 이메일 도메인입니다. 문의사항이 있으시면 이메일로 연락주세요.",
  },
};
...
> **참고**: 현재 개발 단계에서는 RLS가 비활성화되어 있습니다. 프로덕션 배포 전에는 반드시 RLS를 활성화하고 적절한 정책을 설정하세요.

#### 개발 환경 (현재 설정)

개발 중에는 RLS가 비활성화되어 있어 권한 관련 에러 없이 빠르게 개발할 수 있습니다:
...
프로덕션 배포 시에는 반드시 RLS를 활성화하고 다음 정책을 적용하세요:
...
CREATE TABLE tasks (
  ...
  -- user_id는 Clerk user ID를 저장 (auth.jwt()->>'sub'와 비교)
  CONSTRAINT fk_user FOREIGN KEY (user_id) REFERENCES users(clerk_id)
);

-- RLS 활성화
ALTER TABLE tasks ENABLE ROW LEVEL SECURITY;

-- SELECT: 사용자는 자신의 작업만 조회 가능
CREATE POLICY "Users can view their own tasks"
  ON tasks FOR SELECT
  TO authenticated
  USING (auth.jwt()->>'sub' = user_id);

-- INSERT: 사용자는 자신의 작업만 생성 가능
CREATE POLICY "Users can insert their own tasks"
  ON tasks FOR INSERT
  TO authenticated
  WITH CHECK (auth.jwt()->>'sub' = user_id);

-- UPDATE: 사용자는 자신의 작업만 수정 가능
...
- `auth.jwt()->>'sub'`: Clerk 세션 토큰에서 사용자 ID를 추출합니다
- `TO authenticated`: 인증된 사용자에게만 정책이 적용됩니다
- `USING`: SELECT, UPDATE, DELETE에서 기존 데이터를 필터링합니다
- `WITH CHECK`: INSERT, UPDATE에서 새 데이터를 검증합니다

**4. RLS 정책 테스트**

RLS 정책이 올바르게 작동하는지 테스트하려면:

1. 두 개의 다른 Clerk 계정으로 로그인
2. 각 계정에서 데이터를 생성하고 조회
3. 다른 사용자의 데이터는 조회되지 않아야 함

### Supabase 연결 및 데이터 쿼리

이 프로젝트는 [Supabase 공식 Next.js 가이드](https://supabase.com/docs/guides/getting-started/quickstarts/nextjs)의 모범 사례를 따릅니다.

**Server Component에서 데이터 쿼리:**
...
- `@supabase/ssr` 패키지 사용으로 cookie 기반 세션 관리
- Next.js 15의 `await cookies()` 패턴 적용
- Clerk 인증과 자동 통합 (`accessToken` 옵션)
- Server Component에서 최적화된 데이터 페칭

**예시 페이지:**

- `/instruments`: Supabase 공식 가이드의 예시를 기반으로 한 데이터 쿼리 데모
